$('.comics-area').each(function () {
    var $table = $(this);
    var itemsPerPage = 10;
    var currentPage = 0;
    var pages = Math.ceil($table.find("a").length / itemsPerPage);
    ...
    $table.find(
    'a').hide().slice(currentPage * itemsPerPage, (currentPage + 1) * itemsPerPage).show();
    });
    ...
    $(document).scroll(function() {
        ...
        if (topy > 400) {
            $('a.top').addClass('appear');
        } else {
            $('a.top').removeClass('appear');
        }
    });

    $(document).ready(function(){
        $('a.top').click(function(){ 
            $('html,body').animate({scrollTop:$('#sectop').offset().top}, 500);
        });
    });